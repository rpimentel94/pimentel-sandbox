 ...
 * )
 */
class MediaAlter extends SqlBase {
  ...
  public function prepareRow(Row $row) {
    ...
    $og_uuid = $this->getDatabase()->query('
        SELECT
        mfd.thumbnail__target_id,
        m.uuid
        FROM
        {media_field_data} mfd
        JOIN
        {media} m 
        ON 
        mfd.mid = m.mid
        WHERE
        mfd.thumbnail__target_id = :fid
    ', array(':fid' => $fid));

    if ($og_uuid) {
        foreach ($og_uuid as $record) {
            $row->setSourceProperty('uuid', $record->uuid);
        }
    }
